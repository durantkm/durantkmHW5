...
#Gets Database password so shell can fucntion
...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:"+database_password+"@localhost:5432/durantkmHW5"
...
# TODO: Write a send_email function here. (As shown in examples.)--Completed
...
# TODO: Add and run a  migration so that each twitter username also saves an associated email.--completed
# User model
class User(db.Model):
    __tablename__ = "users"
    ...
    def __repr__(self):
        return "{} (ID: {})".format(self.twitter_username,self.id, self.user_email)
...
#completed  TODO: Add a field in the form for a user to enter their email (the email that goes with the twitter username entered).
#completed  TODO: Edit the template that renders the form so that email is also asked for!
class TweetForm(FlaskForm):
    ...
    email = StringField("Please provide your email", validators=[Required()])
...
#completed TODO: Edit get_or_create_user (AND get_or_create_tweet -- see below) as necessary to store a user's email 
#as well as their twitter username. The get_or_create_user function should accept an email as input and 
#deal with it appropriately to save it as part of a User row. Each user (from now on) has an email! 
#This should be a small change to how the function currently works.
...
#completed TODO: You will need to make changes in this function as well, to address users having emails. See above. What do you need to change to make sure get_or_create_user is *invoked* correctly, including saving an email? Does anything need to change about the input to get_or_create_tweet, and *its* invocations?
...
#completed TODO: Edit the index route so that, when a tweet is saved by a certain user, that user gets an email. 
#Use the send_email function (just like the one in the textbook) that you defined above.
# NOTE: You may want to create a test gmail account to try this out so testing it out is not annoying. 
#You can also use other ways of making test emails easy to deal with, as discussed in class!
...
def index():
    ...
    if form.validate_on_submit():
    	user = User.query.filter_by(twitter_username= form.username.data).first()
    	if user is None:
    		user = get_or_create_user(db.session, form.username.data, form.email.data)
    		if app.config['FLASKY_ADMIN']:
    			send_email(app.config['FLASKY_ADMIN'], 'New User','new_user_msg', user=user)
